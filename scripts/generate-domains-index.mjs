...
const TOOL_DOMAIN_OUT_FILE = path.resolve('src/server/registry/generated-tool-domains.ts');
const require = createRequire(import.meta.url);

// The catalog embeds a JSON.stringify blob, which is not oxfmt-style (double
// quotes, quoted keys). Lefthook pre-commit formats staged .ts files, so the
// committed copy is always oxfmt-formatted — reformat generated output here to
// match, otherwise every build rewrites ~55k lines and dirties the worktree.
// generated-domains.ts is already emitted oxfmt-clean; formatting it is a no-op.
function formatGeneratedFiles() {
  ...
  if (!existsSync(bin)) {
    console.warn('[generate-domains-index] oxfmt not found; generated files left unformatted.');
    return;
  }
  const result = spawnSync(bin, [OUT_FILE, CATALOG_OUT_FILE, TOOL_DOMAIN_OUT_FILE, '--write'], {
    cwd: path.resolve('.'),
    encoding: 'utf8',
  ...
  if (result.status !== 0) {
    ...
    console.warn(
      `[generate-domains-index] oxfmt failed (status=${result.status}); generated files left unformatted.${details ? `\n${details}` : ''}`,
    );
  }
}
...
async function main() {
  ...
  const toolDomainPairs = catalog.entries.map((entry) => [entry.tool.name, entry.domain]);
  const domainNames = [...new Set(catalog.entries.map((entry) => entry.domain))].toSorted((a, b) =>
    a.localeCompare(b),
  );
  const toolDomainSource = `// AUTO-GENERATED BY scripts/generate-domains-index.mjs
// DO NOT EDIT DIRECTLY
 ...
 */
export const GENERATED_TOOL_DOMAIN_MAP: Readonly<Record<string, string>> = ${JSON.stringify(Object.fromEntries(toolDomainPairs), null, 2)};

/** Every domain that owns at least one built-in tool, sorted. */
export const GENERATED_DOMAIN_NAMES: readonly string[] = ${JSON.stringify(domainNames, null, 2)};
`;
  await fs.writeFile(TOOL_DOMAIN_OUT_FILE, toolDomainSource, 'utf-8');

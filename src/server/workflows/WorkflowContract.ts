/** Declarative workflow contract shared by the runtime and extension SDK. */
...
/** Tool inputs may be literal values or `${step.field}` template strings. */
...
export interface WorkflowContract {
   ...
   * successfully. Consumed by workflow_suggest to recommend next steps from
   * the client-supplied executed list.
   */
  readonly chainsWith?: string[];
  /**
   * Workflow ids that should complete before this workflow runs. Consumed by
   * workflow_suggest to surface missing prerequisites per suggestion.
   */
  readonly prerequisites?: string[];
...
function createToolBuilder(id: string, toolName: string): ToolBuilder {
  ...
  let input: Record<string, ToolNodeInput> | undefined;
